            ...
            <div class="card mg-b-20">
                <div class="card-body">
                    <div class="row row-xs align-items-center mg-b-20">
                        <div class="col-md-4">
                            <div class="main-img-user"><img alt="" src="{{ $courses->image != '' ? asset($courses->image) : asset('dashboard/img/users/6.jpg') }}" class=""></div>
                        </div>

                    </div>
                    <form id="fileUploadForm" class="needs-validation was-validated" action="{{route('courses.update',$courses->id)}}" method="post" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="row row-sm">
                            <div class="form-group col-md-6 has-success mg-t-10">
                                <label for="title_ar">{{ __('messages.name in arabic') }}  :</label>
                                <input id="title_ar" type="text" class="form-control " name="title_ar" value="{{$courses->title_ar}}"  required>
                                @error('title_ar')
...
{{--                                <input id="name_en" type="text" class="form-control " name="title_en" value="{{$courses->title_en}}"  required>--}}
{{--                                @error('name_en')--}}
{{--                                <span class="text-danger">{{ $message }}</span>--}}
{{--                                @enderror--}}
{{--                            </div>--}}
                            ...
                            <div class="form-group col-md-6 has-success mg-t-10">
                                ...
                                <select class="testselect2" required="" name="is_paid">
                                    ...
                                    <option value="0" @if($courses->is_paid != 1) selected @endif>Free</option>
                                </select>
                            </div>
